...
def filter_and_prepare_places(places, visited, planned, available_budget, location, radius):
    ...
    for p in places:
        ...
        if not name or not lat or not lng:
            print(f"Skipping place due to missing data: {p}")
            continue
        ...
        if available_budget is not None and cost_estimate > available_budget:
            continue
        
        # Ensure the place is within the radius
        distance = haversine_distance(location[0], location[1], lat, lng) * 1000  # km to meters
        if distance is None or (radius is not None and distance * 1000 > radius):
            continue
    ...
    print(f"Filtered places: {json.dumps(results, indent=4)}")  # Debugging output data
    return results
