...
def approximate_travel_time_min(origin, destination, speed_kmh=AVERAGE_SPEED_KMH):
    """
    Approximate travel time (in minutes) by computing haversine distance (km)
    and dividing by speed (km/h). Ensures a minimum travel time.
    """
    lat1, lng1 = origin
    ...
    return max(time_hours * 60.0, MIN_TRAVEL_TIME)  # ensure minimum travel time
...
def build_itinerary_timeline(itinerary):
    ...
    for entry in itinerary:
        ...
        if entry["type"] == "travel":
            print(f"  Travel from {entry['from_place']} to {entry['to_place']} "
...
def build_itinerary_json(itinerary, itinerary_name="Itinerary_Trip", day=1):
    ...
    for entry in itinerary:
        ...
        if entry["type"] == "travel":
            location = f"Travel from {entry['from_place']} to {entry['to_place']}"
            activity = "Travel"
            activity_desc = f"Travel from {entry['from_place']} to {entry['to_place']}."
        elif entry["type"] == "activity":
